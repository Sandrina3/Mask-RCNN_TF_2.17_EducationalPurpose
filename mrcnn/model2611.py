...
class DetectionTargetLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        gt_masks = tf.cast(inputs[3], tf.bool)
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Ensure target_bbox is float32 to match pred_bbox
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    print('mask_shape',mask_shape.shape)
...
class MaskRCNN(object):
    ...
    def compile(self, learning_rate, momentum):
        """Prepare the model for training (no symbolic loss layers)."""
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            losses["mrcnn_class_loss"] = mrcnn_class_loss_graph(input_gt_class_ids, mrcnn_class_logits,
                                                                KL.Lambda(lambda x: parse_image_meta_graph(x)[
                                                                    "active_class_ids"])(input_image_meta))
            losses["mrcnn_bbox_loss"] = mrcnn_bbox_loss_graph(input_gt_boxes, input_gt_class_ids, mrcnn_bbox)
            print('start mrcnn_mask_loss')
            print(input_gt_masks.shape,input_gt_class_ids.shape,mrcnn_mask.shape)
            losses["mrcnn_mask_loss"] = mrcnn_mask_loss_graph(input_gt_masks, input_gt_class_ids, mrcnn_mask)
            print('end mrcnn_mask_loss')
...
def batch_pack_graph(x, counts, num_rows):
    """Picks different number of values from each row in x depending on counts."""
    outputs = []
    for i in range(num_rows):
        # Use tf.slice: start at 0, take counts[i] rows
        output = tf.slice(x[i], [0, 0], [counts[i], -1])
        outputs.append(output)
    return tf.cast(tf.concat(outputs, axis=0), tf.float32)
...
class NormBoxesLayer(tf.keras.layers.Layer):
    ...
    def compute_output_shape(self, input_shape):
        # input_shape[0] corresponds to boxes shape
