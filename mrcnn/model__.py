...
class MaskRCNN(object):
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            ...
            #loss_tensor = tf.reduce_mean(layer.output) * weight
            #loss_tensor = KL.Lambda(lambda x, w=weight: tf.reduce_mean(x) * w,name=name + "_loss_lambda")(layer.output)
            ...
            #self.keras_model.add_loss(lambda layer_output=layer.output, w=weight:tf.reduce_mean(layer_output) * w)
        ...
        for name in loss_names:
            ...
            weight = self.config.LOSS_WEIGHTS.get(name, 1.0)
            # Add metric via zero-arg callable
            self.keras_model.add_metric(
                lambda layer_output=layer.output, w=weight:
                tf.reduce_mean(layer_output) * w,
                name=name,
                aggregation="mean"
            )
